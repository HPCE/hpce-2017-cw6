...
{
   DX02_state *state = (DX02_state*) vsta;
   DX02_param *param = (DX02_param*) vpar;
   double Z;
   ++state->s;
   ...
   unsigned long S[], int id)
{
   ...
   state = (DX02_state *)util_Malloc (sizeof (DX02_state));
   param = (DX02_param *)util_Malloc (sizeof (DX02_param));
   if (0 == id) {
   ...
   state->X = (double*)util_Calloc (MASK1 + 1, sizeof (double));
   for (j = 0; j < k; j++)
      state->X[k - j - 1] = S[j] % m;
   state->s = k - 1;
   state->K = k;
   state->co = 0; // DT10 : Moving from static variable
   param->b = b;
   param->m = m;

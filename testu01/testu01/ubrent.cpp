...
/* SAFE */ static std::atomic_flag co1 = ATOMIC_FLAG_INIT, co2 = ATOMIC_FLAG_INIT, co3 = ATOMIC_FLAG_INIT, co4 = ATOMIC_FLAG_INIT, co5 = ATOMIC_FLAG_INIT;      /* Counters */
...
{
  /* SAFE (co?) */ static unsigned int w, x[r], weil = 0x61c88647;
   unsigned int t, v;
   /* SAFE (co?) */ static int i = -1;             /* i < 0 indicates first call */
   // DT10 : Not sure about variable i in a more general sense (not thread safety)
...
{
   Xorgen32_param *param = (Xorgen32_param*) vpar;
   Xorgen32_state *state = (Xorgen32_state*) vsta;
   unsigned int t, v;
                                    ...
                                    lebool hasWeyl, unsigned int seed)
{
   ...
   param = (Xorgen32_param*) util_Malloc (sizeof (Xorgen32_param));
   state = (Xorgen32_state*) util_Malloc (sizeof (Xorgen32_state));
...
void ubrent_DeleteXorgen32 (unif01_Gen *gen)
{
   ...
   if (NULL == gen)
      return;
   state = (Xorgen32_state*)gen->state;
   util_Free (state->x);
   gen->state = util_Free (gen->state);
   gen->name = (char *)util_Free (gen->name);
   util_Free (gen);
...
{
  /* SAFE (co?) */ static ulonglong w, x[r],
      weil = ((longlong) 0x61c88646 << 32) + (longlong) 0x80b583eb;
   ulonglong t, v;
   
   /* SAFE (co?) */static int i = -1;             /* i < 0 indicates first call */
   // DT10 : actually, this looks a bit troublesome, as the original behaviour is unclear. TODO
...
unif01_Gen * ubrent_CreateXor4096l (ulonglong seed)
{
   ...
   auto co2Prev=co2.test_and_set();

   util_Assert (!co2Prev,
   "ubrent_CreateXor4096l:\n   only 1 such generator can be used at a time");

   gen = (unif01_Gen*) util_Malloc (sizeof (unif01_Gen));
   pseed = (ulonglong*) util_Malloc (sizeof (ulonglong));
   *pseed = seed;
...
{
   Xorgen64_param *param = (Xorgen64_param*) vpar;
   Xorgen64_state *state = (Xorgen64_state*) vsta;
   ulonglong t, v;
                                    ...
                                    lebool hasWeyl, ulonglong seed)
{
   ...
   param = (Xorgen64_param*) util_Malloc (sizeof (Xorgen64_param));
   state = (Xorgen64_state*) util_Malloc (sizeof (Xorgen64_state));
   ...
   state->x = (ulonglong*) util_Calloc ((size_t) state->r2, sizeof(ulonglong));
...
void ubrent_DeleteXorgen64 (unif01_Gen *gen)
{
   ...
   if (NULL == gen)
      return;
   state = (Xorgen64_state*)gen->state;
   util_Free (state->x);
   gen->state = util_Free (gen->state);
   gen->name = (char *)util_Free (gen->name);
   util_Free (gen);
...
static double xor4096d_U01 (void *junk, void *vsta)
{
  /* SAFE (co?) */static ulonglong w, x[r],
      weil = ((longlong) 0x61c88646 << 32) + (longlong) 0x80b583eb;
   ulonglong t, v;
   static const double t53 = (double) 1 / (double)((longlong) 1 << 53); /* 0.5**53*/
   /* SAFE (co?) */static int i = -1;             /* i < 0 indicates first call */
   // DT10 : Same issue with i. TOD
...
#define ws   (27*UINT64 +  16*UINT32)
  ...
  /* SAFE (co?) */ static UINT w, weyl, zero = 0, x[r];
   UINT t, v;
   /* SAFE (co?) */ static int i = -1;             /* i < 0 indicates first call */
   // DT10 : Same problem with i. TODO
...
unif01_Gen * ubrent_CreateXor4096i (unsigned long seed)
{
   ...
   auto co4Prev=co4.test_and_set();

   util_Assert (!co4Prev,
      "ubrent_CreateXor4096i:\n   only 1 such generator can be used at a time");

   gen = (unif01_Gen*) util_Malloc (sizeof (unif01_Gen));
   pseed = (unsigned long*)util_Malloc (sizeof (unsigned long));
   *pseed = seed;
